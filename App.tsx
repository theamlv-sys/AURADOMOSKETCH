...
const STYLE_PRESETS: StylePreset[] = [
  ...
  { id: 'cartoon_mix', name: 'Cartoon Mix', prompt: 'STRICT_LOCKDOWN: ULTIMATE CARTOON HYBRID. Fusion of adult sci-fi thin-line precision (Rick and Morty style), iconic yellow-saturated skin tones (Simpsons style), and sharp urban anime aesthetics (Boondocks style). MANDATORY: Rendered with high-budget Pixar-quality 3D volumetric lighting, cinematic surface scattering, perfectly smooth textures. FORBIDDEN: Realism, photographic skin, real-world human features.', thumbnail: '📺' },
  { id: 'bighead', name: 'Big Head Mode', prompt: 'STRICT_LOCKDOWN: 3D CARICATURE. Ultra-exaggerated cartoon. MANDATORY: Massive head (5x normal size), tiny micro body, giant feet, high-budget 3D Disney/Pixar render, vibrant colors. FORBIDDEN: Realistic proportions, normal anatomy, flat 2D.', thumbnail: '🦒' },
  ...
  { id: 'cyber', name: 'Cyberpunk', prompt: 'STRICT_LOCKDOWN: 2D CYBERPUNK CARTOON. Bright shiny comic book style. CRITICAL: This is a 2D ANIMATED CARTOON, not reality. Convert all humans into highly-stylized, cel-shaded anime/comic cartoon characters with cybernetic implants. NO REALISTIC HUMANS, NO PHOTOGRAPHY. Electric neon outlines, flat cel-shading colors, and intense glowing tech-wear.', thumbnail: '🌆' },
  { id: 'ghibli', name: 'Studio Ghibli', prompt: 'STRICT_LOCKDOWN: MAGICAL REALISM GHIBLI. High-detail anime feature film. MANDATORY: Lush hand-painted watercolor backgrounds, soft cinematic sunlight, nostalgic atmosphere, highly detailed cel-animated characters, deep atmospheric perspective. FORBIDDEN: 3D CGI, photorealism, raw photography, dull colors.', thumbnail: '🍃' },
  { id: 'pixel', name: 'Pixel Art', prompt: 'STRICT_LOCKDOWN: RETRO 16-BIT VIDEO GAME CARTOON. Flat pixel art. CRITICAL: Convert the entire image into a low-resolution cartoon pixel art sprite scene. Humans MUST become blocky, exaggerated, flat 2D video game characters. NO REALISTIC SHADING, NO PHOTOGRAPHY, NO HIGH DEFINITION. Use a limited 16-color palette and absolute rigid square pixels only.', thumbnail: '👾' },
  { id: 'oil', name: 'Oil Painting', prompt: 'STRICT_LOCKDOWN: OIL ON CANVAS. Impasto. MANDATORY: Thick visible brushstrokes, canvas weave, wet oil sheen. FORBIDDEN: Digital smoothness, photorealism.', thumbnail: '🎨' },
  { id: 'photo', name: 'Hyper-Real', prompt: 'STRICT_LOCKDOWN: RAW PHOTOGRAPHY. Extreme detail. MANDATORY: Skin pores, realistic lighting, raw unedited look. FORBIDDEN: Painterly effects, cartoon traits.', thumbnail: '📸' },
  { id: 'surreal', name: 'Surrealism', prompt: 'STRICT_LOCKDOWN: SURREALIST ART. Dream logic. MANDATORY: Bizarre juxtapositions, melting forms. FORBIDDEN: Normal reality, logic.', thumbnail: '🫠' },
  { id: 'water', name: 'Watercolor', prompt: 'STRICT_LOCKDOWN: FINE WATERCOLOR. Fluid pigments. MANDATORY: Soft bleeding edges, wet-on-wet textures. FORBIDDEN: Sharp digital lines, photorealism.', thumbnail: '🖌️' },
  { id: 'dark', name: 'Dark Fantasy', prompt: 'STRICT_LOCKDOWN: GOTHIC FANTASY. Eldritch aesthetic. MANDATORY: Gritty textures, ominous fog, rusted metal. FORBIDDEN: Bright cheerful colors.', thumbnail: '⚔️' },
  { id: 'ue5', name: '3D Render', prompt: 'STRICT_LOCKDOWN: 3D ANIMATED CARTOON MOVIE. Pixar/Dreamworks style. CRITICAL: This is a stylized 3D CARTOON, not reality. Convert all humans into cute, exaggerated, highly-stylized 3D cartoon characters with smooth plastic skin, big expressive cartoon eyes, and soft stylized clothing. NO REALISTIC HUMANS, NO PHOTOGRAPHY. Bright, cinematic 3D lighting with perfectly smooth, stylized CGI geometry.', thumbnail: '🎮' },
  { id: 'pop', name: 'Pop Art', prompt: 'STRICT_LOCKDOWN: VINTAGE POP ART. Graphic impact. MANDATORY: Ben-Day dots, bold primary colors, thick outlines. FORBIDDEN: Realism, soft shading.', thumbnail: '💥' },
  { id: 'pencil', name: 'Pencil Art', prompt: 'STRICT_LOCKDOWN: GRAPHITE SKETCH. Hand-drawn. MANDATORY: Grayscale, lead smudges, cross-hatching. FORBIDDEN: Color, digital perfection.', thumbnail: '✏️' },
  { id: 'street', name: 'Street Art', prompt: 'STRICT_LOCKDOWN: GRAFFITI MURAL. Urban spray. MANDATORY: Drips, stencil layers, brick wall texture. FORBIDDEN: Clean gallery art.', thumbnail: '🏙️' },
  { id: 'abstract', name: 'Abstract Art', prompt: 'STRICT_LOCKDOWN: FINE ABSTRACT. Non-linear. MANDATORY: Dynamic shapes, emotional color usage. FORBIDDEN: Recognizable realistic forms.', thumbnail: '🎨' },
  ...
  { id: 'claymation', name: 'Claymation', prompt: 'STRICT_LOCKDOWN: EXTREME CARTOON CLAYMATION. Play-Doh Diorama. CRITICAL: This is a CRUDE CARTOON, not reality. Convert the ENTIRE image (people, clothes, background) into crude, chunky, brightly-colored modeling clay. Humans MUST be rendered as goofy, thick-proportioned clay cartoons with dot-eyes and featureless blobs for faces/hands. NO REAL FABRIC, NO REAL SKIN, NO PHOTOGRAPHY. Everything is shiny, smudged plasticine with visible thumbprints.', thumbnail: '🧸' }
];
              ...
              <>
                ...
                <div className={`absolute right-0 top-full mt-3 w-64 rounded-2xl border p-2 shadow-2xl z-[200] animate-in fade-in slide-in-from-top-2 duration-200 flex flex-col gap-1 ${theme === 'dark' ? 'bg-[#0a0a0a] border-white/10' : 'bg-white border-slate-200'}`}>
                  ...
                  <button onClick={() => { setShowPlanSelection(true); setIsProfileMenuOpen(false); }} className={`w-full text-left flex items-center justify-between px-3 py-2 text-[11px] font-bold rounded-lg transition-colors group ${theme === 'dark' ? 'text-amber-400 hover:bg-amber-400/10' : 'text-amber-600 hover:bg-amber-50'}`}>
                    ...
                    <span className={`text-[8px] px-1.5 py-0.5 rounded-full border uppercase tracking-widest ${theme === 'dark' ? 'border-amber-500/30' : 'border-amber-600/30'}`}>{userTier || 'none'}</span>
                  </button>
              ...
              <>
                ...
                <div className={`absolute right-0 top-full mt-2 w-48 rounded-xl border p-2 flex flex-col gap-1 shadow-2xl z-[102] animate-in slide-in-from-top-2 duration-200 ${theme === 'dark' ? 'bg-[#111111] border-white/10' : 'bg-white border-slate-200'}`}>
                  ...
                  <button onClick={() => { setShowPlanSelection(true); setIsMobileMenuOpen(false); }} className={`flex w-full items-center justify-between px-4 py-3 rounded-lg text-xs font-bold uppercase tracking-widest ${theme === 'dark' ? 'text-amber-400 hover:bg-white/5' : 'text-amber-600 hover:bg-slate-50'}`}>
                    ...
                    <span className="text-[10px] opacity-60">({userTier || 'none'})</span>
                  </button>
