...
// 2. Generate Image (Veo Studio Image Mode) - Native Text/Image to Image
app.post('/api/generate-image-mode', async (req, res) => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
        const { prompt, images, modelMode, aspectRatio, resolution } = req.body;
        ...
        const model = modelMode === 'pro' ? 'gemini-3-pro-image-preview' : 'gemini-2.5-flash-image';

        // Enforce image limits
        const maxImages = modelMode === 'pro' ? 5 : 3;
        const validImages = Array.isArray(images) ? images.slice(0, maxImages) : [];

        // Build contents array for the API
        const contents = [];
        contents.push({ text: prompt });

        // Add base64 images to contents
        ...
        console.log(`[Image Mode] Calling ${model} with ${validImages.length} images...`);
        ...
        if (modelMode === 'pro' && resolution) {
            genConfig.imageSize = resolution; // '1K', '2K', '4K'
        }
        ...
        const candidate = response.candidates?.[0];
        if (candidate?.finishReason === 'SAFETY') {
            throw new Error("AI Refusal: Content blocked by safety filters.");
        }

        const parts = candidate?.content?.parts;
        if (!parts || parts.length === 0) {
            throw new Error("No content generated by model. (Empty Response)");
        }
        ...
        console.error('[Image Mode] Generation Error:', err);
        return res.status(500).json({ error: err.message });
    }
});
